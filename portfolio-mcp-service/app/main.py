...
- 데이터 한계·민감정보: 계좌번호(account_no)·detail 의 식별자는 가운데가 가려진 채(`[계좌번호 일부 가려짐]`) 제공된다 — 그대로 두고 복원하려 하지 마라. 통화가 섞이면(KRW·USD) 합산 수치는 단순합이라 환산 전제 없이 비교하지 말고 통화를 함께 밝혀라. 결과가 잘렸으면(truncated) "최근 250건 기준"이라 밝히고 범위를 좁히도록 권하며 총건수·전수집계를 단정하지 마라.
