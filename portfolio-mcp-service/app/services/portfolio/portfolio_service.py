...
class PortfolioService:
    ...
    async def list_accounts(self) -> list[dict]:
        ...
        for a in accounts:
            ...
            market_value = sum(float(h.get("quantity") or 0) * float(h.get("last_price") or 0) for h in holdings)
            ...
            out.append(
                {
                    ...
                    "base_currency": a.get("base_currency") or "KRW",
                    "nav": round(market_value + cash, 2),
    ...
    async def list_holdings(
        ...
        for acc_id, holdings in holdings_by_acc.items():
            ...
            acc_total = sum(line["market_value"] for line in lines) or 1.0
            for line in lines:
                line["weight"] = round(line["market_value"] / acc_total * 100, 2)
            rows.extend(lines)
        ...
        for acc_id, txs in tx_by_acc.items():
            for t in txs:
                ...
                if tx_type and line["tx_type"] != tx_type:
                    continue
                if kws and not any(k.lower() in line["ticker"].lower() or k.lower() in line["name"].lower() for k in kws):
                    continue
                rows.append(line)
        rows.sort(key=lambda r: r["trade_date"])  # 시간순(오래된→최신)
        truncated = len(rows) > _MAX_RESULTS
        rows = rows[-_MAX_RESULTS:] if truncated else rows
        return {
            "transactions": rows,
        ...
        for acc_id, orders in orders_by_acc.items():
            for o in orders:
                ...
                if kws and not any(k.lower() in line["ticker"].lower() or k.lower() in line["name"].lower() for k in kws):
                    continue
                rows.append(line)
        rows.sort(key=lambda r: r["placed_at"], reverse=True)  # 최신 접수순
        truncated = len(rows) > _MAX_RESULTS
    ...
    async def get_account_activity(self, account_id: str, since: str | None = None, until: str | None = None) -> dict:
        ...
        for t in txs:
            ...
            action = "dividend" if line["tx_type"] == "dividend" else ("cash" if line["tx_type"] in cash_types else "trade")
