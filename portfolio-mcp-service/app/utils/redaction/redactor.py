...
# 값이 공백으로 쪼개진 경우('5012 01 2345678')의 이어지는 숫자 토큰.
# 같은 줄로 한정하고(`[^\S\n]`) 날짜 토큰은 제외해, 값 뒤에 오는 무관한 날짜·수치를 삼키지 않는다.
_VALUE_CONT = rf"(?:[^\S\n]+(?!{_DATE_AHEAD})\d[\d-]*)*"
...
_PATTERNS: list[tuple[re.Pattern, object]] = [
    ...
    # 자릿수는 공백으로 쪼개져도 세므로 '5012 01 2345678' 도 값 전체가 한 번에 걸린다).
    (
        re.compile(
            r"(?i)\b(?:account[_-]?no|account[_-]?number|계좌번호|카드번호)\b"
            rf"(?:\s*[:=：]\s*\S+{_VALUE_CONT}|\s+(?=(?:[^\S\n]*[\d-]){{6}})\d[\d-]*{_VALUE_CONT})"
        ),
        _MASK,
    ),
