...
_PATTERNS: list[tuple[re.Pattern, object]] = [
    ...
    # 구분자는 :/= 뿐 아니라 공백도 받는다(공백일 땐 값이 6자리+ 숫자형일 때만 — 오탐 방지).
    (
        re.compile(r"(?i)\b(?:account[_-]?no|account[_-]?number|계좌번호|카드번호)\b(?:\s*[:=]\s*\S+|\s+\d[\d-]{5,})"),
        _MASK,
    ),
    # 카드번호 (16자리, 4-4-4-4 / 공백 / 연속) — 전체 카드번호는 부분 마스킹
    (re.compile(r"\b\d{4}[ -]?\d{4}[ -]?\d{4}[ -]?\d{4}\b"), _mask_account_no),
    # 브로커 계좌번호 (3~4 그룹, 하이픈 구분, 예: 123-45-678901 / 5012-01-2345678)
    (re.compile(r"\b\d{2,6}-\d{2,6}-\d{2,8}(?:-\d{1,8})?\b"), _mask_account_no),
]
