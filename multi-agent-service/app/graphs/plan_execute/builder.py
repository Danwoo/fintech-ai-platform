"""Plan-then-Execute StateGraph 빌더.
...
from graphs.plan_execute.compliance import _ensure_disclaimer
from graphs.plan_execute.domain_map import (
    _DOMAIN_LABELS,
    _classify_domain,
...
from graphs.plan_execute.executor import _invoke_agent_safe
from graphs.plan_execute.formatting import (
    _build_history_ctx,
    _extract_query,
    _format_all_results_for_answer,
    _format_prior_stage_results,
)
from graphs.plan_execute.schemas import ClarifyDecision, ExecutionPlan
from graphs.plan_execute.stages import _normalize_stages
    ...
    async def _run_stage_node(state: PlanExecuteState, config: RunnableConfig) -> dict:
        ...
        for task_item in stage_tasks:
            ...
            if agent is None:
                ...
                continue
            # 원본 쿼리를 항상 포함 — 플래너가 짧은 task 를 생성필도 도메인 에이전트가 검색 맥락 확보
