...
- `clients/file/sftp_client.py` → `SftpClient`: asyncssh SFTP (file-service / doc-search 이미지 파이프라인). 서비스별 추가 클라(doc-search `clients/milvus/milvus_client.py`[pymilvus 연결]·`clients/llm/`·`clients/upstage/`, market-data-mcp `clients/market/`[시장 데이터 벤더 HTTP 프록시] 등)는 해당 서비스에만 존재. ※ Milvus 연결 client(`get_milvus_client`)는 데이터접근 repository(`DocChunkMilvusRepository`)에 주입 — SQL engine→repo 와 동일 구조
